...
class SuperResolution:
	...
	def build_optimizer(self):
      ...
      #L(θ) = αl1(θ) + (1 − α)l2(θ) + β||θ||2 = loss1 + loss2 + loss3
	...
	def print_status(self, mse, adress, name_file_result):
    ...
    ### Sauvegarde données :
    #path_svg_model = '/content/drive/MyDrive/SYS843/Github/sr/DRCN/svg_models/srcnn_x' + str(args.scale) + '_epoch' + str(epoch) + 'sur' + str(args.num_epochs) + '_lr' + str(args.lr) + '_batch' + str(args.batch_size)
    #torch.save(model.state_dict(), path_svg_model)
		...
		with open(adress + name_file_result, 'a', encoding='UTF8', newline='') as f:
			...
			writer.writerow([self.epochs_completed, mse, psnr, self.training_psnr, self.lr]) #, type(self.loss_alpha), type(self.loss1), type(self.loss2), type(self.loss3)
	...
	def do(self, input_image):
		...
		y = np.multiply(y[0], 255.0 / self.max_value)
