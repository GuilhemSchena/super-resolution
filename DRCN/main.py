...
flags.DEFINE_float("lr_decay", 0, "Learning rate decay rate when it does not reduced during specific epoch") #0.5
flags.DEFINE_integer("lr_decay_epoch", 0, "Decay learning rate when loss does not decrease") #4
...
flags.DEFINE_integer("feature_num", 96, "Number of CNN Filters")  #96
flags.DEFINE_integer("cnn_size", 3, "Size of CNN filters")
flags.DEFINE_integer("inference_depth", 9, "Number of recurrent CNN filters") #9
...
# Training or Others
flags.DEFINE_boolean("is_training", True, "Train model with 91 standard images") #True
...
#maj epochs
flags.DEFINE_integer("nb_epochs", 300, "Number of total epochs")
...
def main(_):
  ...
  # Charger les images de test/training
  ...
  # Création des trois parties du modèle
...
def train(training_filenames, test_filenames, model):
  ...
  adress = '/content/drive/MyDrive/SYS843/Github/sr/DRCN/results/'
  name_file_result = 'result_DRCN_x' + str(model.scale) + '_nbepochs' + str(FLAGS.nb_epochs) + '_endlr' + str(FLAGS.end_lr) + '_batch' + str(model.batch_size) + '_featurenum' + str(model.feature_num) + '_inferencedepth' + str(model.inference_depth) + '.csv'

  mse = model.evaluate()
  model.print_status(mse, adress, name_file_result)

  #while model.lr > FLAGS.end_lr:
  # Itération par epoch
  while model.epochs_completed < FLAGS.nb_epochs:
    # Entraînement des batchs
